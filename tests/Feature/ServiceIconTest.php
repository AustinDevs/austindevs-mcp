...
test('brand icons resolve from the URL host when the name does not match', function () {
    ...
    expect($icons->resolve('Errors', 'https://mcp.sentry.dev/mcp'))->toBe(url('/img/services/sentry.png'));
    expect($icons->resolve('Deploys', 'https://api.fly.io/mcp'))->toBe(url('/img/services/fly.svg'));
    expect($icons->resolve('Tweets', 'https://mcp.x.com/mcp'))->toBe(url('/img/services/x.svg'));
...
test('connections prefer the fetched favicon then the brand icon then the placeholder', function () {
    ...
    expect($brand->iconUrl())->toBe(url('/img/services/asana.svg'));
    expect($plain->iconUrl())->toBe(url('/img/services/default.svg'));
});
